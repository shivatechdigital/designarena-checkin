...
1. In the Razorpay Dashboard, switch to **Test Mode** and create or copy the Test Key ID and Test Key Secret.
2. On Hostinger, edit `api/config.php` directly. Do not put these credentials in JavaScript or the admin panel.
3. Set only these two lines:

```php
const RAZORPAY_KEY_ID = 'rzp_test_your_key_id';
const RAZORPAY_KEY_SECRET = 'your_test_key_secret';
```

4. Keep `RAZORPAY_KEY_SECRET` private. The application exposes only the Test Key ID to Razorpay Checkout and sends the secret only from PHP to Razorpay for order creation and signature verification.
5. Open a public booking, choose **Pay Online**, select a visible coupon if applicable, and click **Pay Securely**. Use only Razorpay test cards or test UPI details from the Razorpay Dashboard.

Never use live Razorpay credentials until the booking, cancellation/refund policy, payment verification, and email/WhatsApp notifications have been tested in test mode.
